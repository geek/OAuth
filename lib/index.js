...
function authorizeRequestWithClient(authServer, client, context, userId, callback) {
    ...
    if (!context.redirectUri || !authServer.clientService.isValidRedirectUri(client, context.redirectUri)) {
    ...
    if (utils.isCodeResponseType(context.responseType)) {
    ...
    if (utils.isTokenResponseType(context.responseType)) {
...
AuthServer.prototype.authorizeRequest = function (request, userId, callback) {
    ...
    if (!utils.isAllowedResponseType(context.responseType)) {
    ...
    authServer.clientService.getById(context.clientId, function(error, client){
        if(error){
            return callback(error);
        }
...
AuthServer.prototype.getDeviceCode = function (request, callback) {
    ...
    authServer.clientService.getById(context.clientId, getCodeWithClient);
};
...
AuthServer.prototype.getTokenData = function (context, userId, callback) {
    ...
    if (context.grantType === grantTypes.password) {
    ...
    if (context.grantType === grantTypes.clientCredentials) {
        kgo
        ('token', authServer.tokenService.generateToken)
    ...
    if (context.grantType === grantTypes.implicit) {
...
AuthServer.prototype.grantAccessToken = function (request, userId, callback) {
    ...
    else if (!grantTypes.isAllowed(context.grantType, authServer)) {
        return callback(errors.unsupportedGrantType(context.state));
    }
...
AuthServer.prototype.validateAccessToken = function (request, callback) {
    ...
    this.authorizationService.getAccessToken(context.accessToken, function (error, tokenData) {
        if(error){
            return callback(error);
        }
